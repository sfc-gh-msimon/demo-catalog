...
st.write(
"""
## Search for a Customer and their invoices. 
### The PDF documents are being served directly from the Snowflake Stage \
...
def get_data():
    ...
    # We can easily get the customer and invoice info but we also have the unique file name that can be used to join to our directory table (stage). 
    # Once we have our stage we use the get_presigned_url() function to get the AWS URL to our file 
    # we also splice the first and last name and then concatinate them formatted as Last, First. 
...
def get_pdf_url(df):
    ...
    # Select box options - instead of stacking the select boxes we can put them side by side in 2 columns 
    ...
    #and with the invoice name we can get the URL and store it in a variable. the FILE value is the snowflake presigned URL. 
...
def displayPDF(file):
    # Open file from file path. this is used to open the file from a website rather than local
