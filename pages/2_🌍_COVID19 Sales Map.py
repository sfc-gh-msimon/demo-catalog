...
##### 
  ...
  - **Monthly Sales data for the last 30 days for each state**  
    Sales are represented by the blue bars ontop of the red circles.  The higher the bar the higher the sales for the state in the past 30 days. 
"""
)
...
with st.spinner('Generating Map'):
    ...
    cs.execute("SELECT  STATE, SUM_BY_ST, CASE_SUM, LAT, LON  FROM VW_MAP_SALES_COVID_DATA")

    #create a dataframe from the query result 
    df1 = cs.fetch_pandas_all()

    #format our numbers by creating two new rows with commas and $s
    ...
    tooltip = {
        "html":
            "<b>State:</b> {STATE} <br/>"
            "<b>Last 30 Day Sales(bar):</b> {formatted_sum} <br/>"
            "<b>Covid Cases(circle):</b> {formatted_cases} <br/>",
        "style": {
